namespace DreamBook.API.Import;
...
public static class SeedDatabase
{
    ...
    public async static Task Seed(IApplicationBuilder app)
    {
        using (var serviceScope = app.ApplicationServices.GetRequiredService<IServiceScopeFactory>().CreateScope())
        {
            ...
            await CreateUserRoles(serviceScope.ServiceProvider.GetService<RoleManager<Role>>());
            await CreateUser(serviceScope.ServiceProvider.GetService<UserManager<User>>(), context);
        }
    }
    ...
    static async Task CreateUserRoles(RoleManager<Role> roleManager)
    {
        ...
        if (!await roleManager.RoleExistsAsync(UserRoles.Basic))
            await roleManager.CreateAsync(new Role() { Name = UserRoles.Basic });
        if (!await roleManager.RoleExistsAsync(UserRoles.Moderator))
            await roleManager.CreateAsync(new Role() { Name = UserRoles.Moderator });
    }
