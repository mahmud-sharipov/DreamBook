...
{
    public class DreamTypesController : Controller
    {
        public IDreamTypeService Service { get; }
        public IMapper Mapper { get; }

        public DreamTypesController(IDreamTypeService service, IMapper mapper)
        {
            Service = service;
            Mapper = mapper;
        }
        ...
        public async Task<ActionResult> Create(CreateDreamTypeRequestModel queryModel)
        {
            try
            {
        ...
        {
            var viewModel = await Service.GetByIdWithTranslations(id);
            var queryModel = Mapper.Map<UpdateDreamTypeRequestModel>(viewModel);
            return View(queryModel);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([FromRoute] Guid id, UpdateDreamTypeRequestModel queryModel)
        {
            try
            {
                if (ModelState.IsValid)
